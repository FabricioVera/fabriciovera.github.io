...
| 🟠 **Media** | `src/lib/supabase.ts` | **Credenciales quemadas en código**: Las credenciales de Supabase están hardcodeadas como strings directos en lugar de leer `import.meta.env.PUBLIC_SUPABASE_URL` y `import.meta.env.PUBLIC_SUPABASE_ANON_KEY` de `.env`. |
