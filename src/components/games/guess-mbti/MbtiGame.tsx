...
export default function MbtiGame() {
  ...
  return (
    <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 p-4">
      ...
      <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 flex flex-col items-center text-center">
        <div className="w-48 h-48 mb-4 overflow-hidden rounded-full border-4 border-slate-600 bg-slate-900">
          <img
            ...
            onError={(e) => {
              // Fallback si la imagen de PDB no carga (muy común)
        ...
        {status !== "playing" && (
          <div
            className={`p-4 rounded-lg w-full ${status === "won" ? "bg-green-900/50 border border-green-500" : "bg-red-900/50 border border-red-500"}`}
          >
      ...
      {/* Columna Derecha: Selector de MBTI */}
      <div className="grid grid-cols-4 gap-3 content-start">
        {MBTI_TYPES.map((type) => {
          // Lógica de estilos para feedback visual
          let btnClass =
          ...
          if (status === "playing") {
            ...
            // Estado final (Ganó o Perdió)
            if (type === character.solution) {
              btnClass +=
                "bg-green-600 border-green-400 text-white shadow-[0_0_15px_rgba(34,197,94,0.5)]"; // La respuesta correcta siempre verde
            } else if (type === selectedType && status === "lost") {
              btnClass += "bg-red-600 border-red-400 text-white opacity-50"; // Tu error en rojo
            } else {
              btnClass +=
                "bg-slate-800 border-slate-700 text-slate-600 opacity-30 cursor-not-allowed"; // El resto apagado
            }
          }
